| [Português 🇧🇷](README.md) | [English 🇺🇸](README-en.md) |
|---|---|

# Currículo Digital

## 📌 Sobre o Projeto
Este projeto é um Currículo Digital desenvolvido para apresentar meu perfil profissional, formação acadêmica e habilidades técnicas de forma moderna e acessível. Substituindo os arquivos estáticos tradicionais, esta versão web permite uma experiência mais dinâmica e interativa para recrutadores e colaboradores

## 🚀 Funcionalidades
- Design Limpo e Profissional
- Layout Responsivo
- Organização por Seções
- Links Interativos

## 🛠️ Tecnologias Utilizadas 
- HTML5
- CSS3
- PHP

## 📸 Capturas de Tela
<p align="center">
  <img width="270" height="auto" src="https://github.com/user-attachments/assets/e3036cbc-50e0-452d-8927-b401585e1c63"/>
  <img width="270" height="auto" src="https://github.com/user-attachments/assets/300b6fe4-ecde-4dee-80ba-e70e2a7d1835"/>
  <img width="270" height="auto" src="https://github.com/user-attachments/assets/f1162952-04a1-4b96-a0b2-1eb03d2f412d"/>
</p>
